...
#    return (hcat(minValues[1], maxValues[1]), hcat(minValues[2], maxValues[2]))
...
#    return (hcat(avgValues[1], stdValues[1]), hcat(avgValues[2], stdValues[2]))
...
function normalizeMinMax!(dataset::AbstractArray{<:Real,2})
    ...
    normalizeMinMax!(dataset, mins_maxs)
end
...
function normalizeZeroMean!(dataset::AbstractArray{<:Real,2})
    ...
    normalizeZeroMean!(dataset, avgs_stds);
end
...
function trainClassANN(topology::AbstractArray{<:Int,1},
    ...
    loss_vector = Float32[]
    initial_loss = loss(rna, inputs', targets')
    push!(loss_vector, initial_loss)

    for i in 1:maxEpochs
        Flux.train!(loss, rna, [(inputs', targets')], opt_state)

        current_loss = loss(rna, inputs', targets')
        push!(loss_vector, current_loss)
...
function holdOut(N::Int, P::Real)
    indices = randperm(N);
    num_test = round(Int, N * (1 - P));
    training_indices = indices[1:num_test];
    test_indices = indices[(num_test + 1):N];
...
maxEpochs::Int=1000, minLoss::Real=0.0, learningRate::Real=0.01,
maxEpochsVal::Int=20)
    numInputs = size(trainingDataset[1], 2);
    numOutputs = size(trainingDataset[2], 2);
    rna=buildClassANN(numInputs, topology, numOutputs; transferFunctions);
    loss(rna, x,y) = (size(y,1) == 1) ? Flux.binarycrossentropy(rna(x),y) : Flux.crossentropy(rna(x),y);
    opt_state = Flux.setup(Adam(learningRate), rna);
    ...
    train_loss=loss(rna, trainingDataset[1]', trainingDataset[2]');
    push!(training_loss, train_loss);
    if !isempty(validationDataset[1]) && !isempty(validationDataset[2])
        val_loss=loss(rna, validationDataset[1]', validationDataset[2]');
    ...
    tst_loss = loss(rna, testDataset[1]', testDataset[2]');
    push!(test_loss, tst_loss);

    for i in 1:maxEpochs
        Flux.train!(loss, rna, [(trainingDataset[1]', trainingDataset[2]')], opt_state); 
        train_loss=loss(rna, trainingDataset[1]', trainingDataset[2]');
        push!(training_loss, train_loss);
        tst_loss = loss(rna, testDataset[1]', testDataset[2]');
        push!(test_loss, tst_loss);
    ...
    best_rna = isempty(validationDataset[1]) ? rna : best_rna
    return best_rna, training_loss, validation_loss, test_loss;
end
...
maxEpochsVal::Int=20) 
    ...
    return trainClassANN(topology, (trainingDataset[1], training_matrix); validationDataset=(validationDataset[1], validation_matrix), testDataset=(testDataset[1], test_matrix), transferFunctions, maxEpochs, minLoss, learningRate, maxEpochsVal);
end